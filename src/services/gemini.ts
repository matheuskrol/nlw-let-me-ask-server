...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
  const response = await gemini.models.generateContent({
    ...
    contents: [
      {
        text: `
          Transcreva o áudio para português do Brasil. Seja preciso e natural na transcrição.
...
export async function generateAnswer(
  ...
  const prompt = `
    Com base no texto fornecido abaixo como contexto, responda a pergunta de forma clara e precisa
